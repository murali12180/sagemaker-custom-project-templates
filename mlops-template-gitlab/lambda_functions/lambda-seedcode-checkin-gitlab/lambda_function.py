...
def lambda_handler(event, context):
    ...
    try:
        ...
        build_project.variables.create({'key': 'SAGEMAKER_PROJECT_ARN',
                                        'value': 'arn:aws:sagemaker:' + region + ':' + os.environ[
                                            'AccountId'] + ':project/' + os.environ['SageMakerProjectName']})
    ...
    try:
        ...
        deploy_project.variables.create(
            {'key': 'ARTIFACT_BUCKET', 'value': 'sagemaker-project-' + os.environ['SageMakerProjectId']})
        deploy_project.variables.create({'key': 'SAGEMAKER_PROJECT_ARN',
                                         'value': 'arn:aws:sagemaker:' + region + ':' + os.environ[
                                             'AccountId'] + ':project/' + os.environ['SageMakerProjectName']})
    ...
    for path, _, files in os.walk(model_build_directory):
        for name in files:
            ...
            if name.startswith('._'):
                ...
                try:
                    build_action = {"action": "create", "file_path": full_file_path.split(dir)[1],
    ...
    for path, _, files in os.walk(model_deploy_directory):
        for name in files:
            ...
            if name.startswith('._'):
                ...
                try:
                    deploy = {"action": "create", "file_path": full_file_path.split(dir)[1],
                              "content": open(full_file_path).read()}
